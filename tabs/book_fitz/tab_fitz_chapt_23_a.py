...
"""
KEY_WORDS:  Fitz chapter 23.  23. Querying SQL databases with Qt models  new   tab base stagetwo
...
class Fitz_23_A_Tab( tab_base.TabBase ) :
    ...
    def _build_gui_widgets(self, main_layout  ):
        ...
        widget = QPushButton("test|_msg")
        widget.clicked.connect( self.test_msg  )
        row_layout.addWidget( widget,   )
    ...
    def test_msg( self ):
        ...
        self.append_function_msg( self.msg_widget, "test_msg" )

        for ix in range( 1,30 ):
            msg        = ( f"{ix} test_msg from controller"  )
            #global_vars.CONTROLLER.append_msg( self.msg_widget, msg )
            self.append_msg( msg )


        # print_func_header( "test_msg" )
        # self.append_msg( "test_msg", clear = True )
        # for ix in range( 1,30 ):
        #     self.append_msg( f"{ix} test_msg",   )

    def select_all( self ):
        """ """
        self.append_function_msg( "select_all" )
        self.append_msg( "select_all", clear = True )
